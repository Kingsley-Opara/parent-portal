...
export function ResultsTable() {
  const { academicResult, isLoading, error, retry, downloadCurrentReportCard, isPdfGenerating } = usePortalStore();
    ...
    return (
      <div className="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-7 shadow-xs">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-base font-bold text-slate-900 tracking-tight">Subject Breakdown</h3>
        </div>
        <ResultsEmptyState />
      </div>
    );
      ...
      <div className="rounded-3xl border border-slate-200/80 bg-white p-5 sm:p-7 shadow-xs">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="sm:hidden flex items-center bg-slate-100 p-0.5 rounded-xl border border-slate-200">
              <button
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-200 text-[11px] font-bold text-slate-400 uppercase tracking-wider bg-slate-50/50">
                <th className="py-3 px-3.5 rounded-l-xl">Subject</th>
                <th className="py-3 px-3 text-center">CA1 (20)</th>
                <th className="py-3 px-3 text-center">CA2 (20)</th>
                <th className="py-3 px-3 text-center">Exam (60)</th>
                <th className="py-3 px-3 text-center">Total (100)</th>
                <th className="py-3 px-3 text-center">%</th>
                <th className="py-3 px-3 text-center">Grade</th>
                <th className="py-3 px-3.5 rounded-r-xl">Remark</th>
              </tr>
            </thead>
              ...
              {academicResult.subjects.map((sub) => {
                ...
                return (
                  <tr key={sub.id} className="hover:bg-slate-50/80 transition-colors group">
                    ...
                    <td className="py-3.5 px-3 text-center text-slate-600">{sub.ca1}</td>
                    <td className="py-3.5 px-3 text-center text-slate-600">{sub.ca2}</td>
                    <td className="py-3.5 px-3 text-center text-slate-600 font-medium">{sub.exam}</td>
                    ...
                    <td className="py-3.5 px-3 text-center">
                      <span className={`inline-block px-2.5 py-0.5 rounded-md border text-xs font-bold ${gradeMeta.bg}`}>
                        {sub.grade}
                      </span>
                    </td>
            ...
            <tfoot>
              <tr className="border-t-2 border-slate-200 bg-slate-50 font-bold text-xs text-slate-900">
                <td className="py-3 px-3.5">Aggregate Total</td>
                <td colSpan={3} className="py-3 px-3 text-center text-slate-500 font-normal">
                  {academicResult.subjects.length} Subjects Evaluated
                </td>
                <td className="py-3 px-3 text-center text-indigo-700 font-black text-sm">
                  {academicResult.totalScore}
                </td>
                <td className="py-3 px-3 text-center text-indigo-700 font-black">
                  {academicResult.overallAverage.toFixed(1)}%
                </td>
                <td colSpan={2} className="py-3 px-3.5 text-right text-slate-600 font-medium">
                  Class Position: <strong className="text-slate-900">{academicResult.classPosition} of {academicResult.totalStudentsInClass}</strong>
                </td>
              </tr>
            </tfoot>
          </table>
        ...
        {/* 2. Mobile Responsive Card List (<640px / ~375px mobile friendly) */}
                ...
                >
                  ...
                  <div className="grid grid-cols-4 gap-1.5 text-center text-xs">
                    <div className="bg-white p-2 rounded-xl border border-slate-100">
                      <span className="text-[9px] text-slate-400 font-semibold block uppercase">CA1</span>
                      <span className="font-bold text-slate-700">{sub.ca1}/20</span>
                    </div>
                    <div className="bg-white p-2 rounded-xl border border-slate-100">
                      <span className="text-[9px] text-slate-400 font-semibold block uppercase">CA2</span>
                      <span className="font-bold text-slate-700">{sub.ca2}/20</span>
                    </div>
                    <div className="bg-white p-2 rounded-xl border border-slate-100">
                      <span className="text-[9px] text-slate-400 font-semibold block uppercase">Exam</span>
                    ...
                    <div className="bg-indigo-50/70 p-2 rounded-xl border border-indigo-100">
                      <span className="text-[9px] text-indigo-600 font-semibold block uppercase">Total</span>
                  ...
                  <div className="flex justify-between items-center text-[11px] pt-1 border-t border-slate-200/50">
                    <span className="text-slate-400 font-medium">Teacher Remark:</span>
                    <span className="font-semibold text-slate-700">{sub.remark}</span>
                  </div>
                </div>
              );
            })}
