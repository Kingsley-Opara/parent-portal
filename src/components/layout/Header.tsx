    ...
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200/80 bg-white/95 px-4 sm:px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-slate-900 via-indigo-950 to-indigo-900 text-white shadow-xs group-hover:scale-105 transition-transform">
            <GraduationCap className="h-5 w-5 text-amber-400" />
          </div>
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-base font-bold tracking-tight text-slate-900">Katalysa</span>
      ...
      {/* Center: Active Child Switcher (Desktop Quick Bar) */}
      {children.length > 0 && selectedStudent && (
        <div className="hidden md:flex items-center gap-2">
          <div className="relative">
            <button
              onClick={() => setChildDropdownOpen(!childDropdownOpen)}
              className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50/70 hover:bg-slate-100/80 transition-colors text-left cursor-pointer"
            >
              <div className="relative">
                <img
          ...
          <div className="hidden xl:flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100/70 border border-slate-200/60 text-[11px] text-slate-600 font-medium">
      ...
      {/* Right: Notifications & Parent Profile */}
      <div className="flex items-center gap-2.5">
        {/* Term Info Pill on Mobile/Tablet */}
        <div className="md:hidden flex items-center gap-1 px-2 py-1 rounded-md bg-slate-100 text-[10px] font-medium text-slate-600">
        ...
        {/* Parent Profile Button & Menu */}
        <div className="relative">
          <button
            onClick={() => setProfileDropdownOpen(!profileDropdownOpen)}
            className="flex items-center gap-2 rounded-xl p-1 hover:bg-slate-100 transition-colors cursor-pointer"
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-tr from-indigo-600 to-indigo-800 text-white font-semibold text-xs shadow-xs">
              {user?.firstName?.[0] || "P"}
            </div>
            <div className="hidden sm:flex flex-col text-left">
              ...
              <span className="text-[10px] text-slate-500">Verified Parent</span>
            </div>
          ...
          {profileDropdownOpen && (
            <>
              ...
              />
              <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="border-b border-slate-100 px-3 py-2.5">
                  ...
                  <div className="mt-1 flex items-center gap-1 text-[10px] text-emerald-600 font-medium">
                    ...
                    <span>Active DRF JWT Session</span>
                  </div>
                </div>
